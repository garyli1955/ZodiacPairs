...
class ViewController: UIViewController, AVAudioPlayerDelegate {
    ...
    let marginX = 30
    let marginY = 80
    let cardWidth = 60 //height = 1.35*width
    let cardHeight = 81
    let cardGapX = 20
    let cardGapY = 5
    ...
    override func viewDidLoad() {
        ...
        do {
            ...
            playerClockTick.numberOfLoops = -1 //player forever until beging stopped
    ...
    @IBAction func rememberClicked(_ sender: UIButton) {
        ...
        rememberCounter = REMEMBER_TIMER //TODO: make it configurable
        let tens = rememberCounter / 10
        let digits = rememberCounter % 10
        labelIndicator.text = "\(tens):\(digits)"
    ...
    @objc func rememberTimerProc() {
        if (rememberTimer != nil) {
            ...
            let tens = rememberCounter / 10
            let digits = rememberCounter % 10
            let str = String(format: "%02d:%02d", tens, digits)
            labelIndicator.text = str
        ...
        if gameOver == false && lockCards == false { //process this event only when game is not over
            ...
            if card.isFaceUp == false { //the card has been faced down
                ...
                if firstCard == nil { //no card has been selected
                    ...
                    flipCounter += 1
                    labelIndicator.text = "\(flipCounter)"
                    secondCard = card
